        ...
        .done((data) => {
            ...
            if (data.ok) {
                $.each(data.body, (index, registro) => {
                    ...
                    t.row.add([botonera, registro.id, registro.fecha, registro.placa,registro.infraccion, registro.descripcion]);
                });
                t.draw(false);
            } else {
    ...
    ajaxRequest("GET", `${url}/${id}`)
        .done((data) => {
            if (data.ok) {
                ...
                $("#infraccion").val(data.body.infraccion);
                const fechaISO = data.body.fecha;
                const fechaFormateada = new Date(fechaISO).toISOString().split('T')[0]; // Formato yyyy-mm-dd
...
$(document).ready(function () {
    ...
    $('#liInfraccion').addClass("menu-open");
    $('#liInfracciones').addClass("active");
